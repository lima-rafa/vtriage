...
def test_vcd_wave_sketch_hash_is_stable_for_same_content(tmp_path: Path):
    ...
    (h1, m1) = vcd_wave_sketch_hash(vcd, scope_prefixes=["tb.u_dut.u_core"], tail_event_window=50_000, top_n=12)
    (h2, m2) = vcd_wave_sketch_hash(vcd, scope_prefixes=["tb.u_dut.u_core"], tail_event_window=50_000, top_n=12)
...
def test_vcd_wave_sketch_hash_changes_when_wave_changes(tmp_path: Path):
    vcd = tmp_path / "waves.vcd"
    _write_vcd(vcd, ["#0", "0!", "#10", "1!", "#20", "0!"])
    h1, _ = vcd_wave_sketch_hash(vcd, scope_prefixes=["tb.u_dut.u_core"], tail_event_window=50_000, top_n=12)

    # muda o padrão com mais eventos
    _write_vcd(vcd, ["#0", "0!", "#10", "1!", "#20", "0!", "#30", "1!"])
    h2, _ = vcd_wave_sketch_hash(vcd, scope_prefixes=["tb.u_dut.u_core"], tail_event_window=50_000, top_n=12)
